...
func Close_cmd() {
	...
	err := cmd.Run()
	if err != nil {
		fmt.Println("无法关闭frpc.exe进程", err)
	}
	fmt.Println("成功关闭frpc.exe进程")
}
...
func Copy_to_Clip(port string) string {
	text := IP[0] + ":" + port
	...
	fmt.Println("Sucess to copy to clip!")
	return fmt.Sprintf("Yes")
}

func Noti_Win_True() {
	ip := IP[0] + ":" + REMOTE_PORT
	copy := Copy_to_Clip(REMOTE_PORT)
...
func Noti_Quit_False() {
	notification := toast.Notification{
		AppID:   "Microsoft.Windows.Shell.RunDialog",
		Title:   "SplashNirvana2K",
		Message: "程序仍在运行!!Please Press Close Connect Button!",
		Actions: []toast.Action{
			{"protocol", "My GitHub", "https://github.com/Pumnn1ayLee/PMLfrp-v1.0"},
		},
	}
...
func Noti_Local_False() {
	notification := toast.Notification{
		AppID:   "Microsoft.Windows.Shell.RunDialog",
		Title:   "SplashNirvana2K",
		Message: "Your Local Port is empty!!!",
		Actions: []toast.Action{
			{"protocol", "My GitHub", "https://github.com/Pumnn1ayLee/PMLfrp-v1.0"},
		},
	}
...
func Init_Console(w fyne.Window) {
	...
	connect_button := widget.NewButton("Connect", func() {
		...
		if (KEY == true || KEY == false) && sign_in == false {
			Noti_Set_False()
		}
	})

	//断开连接按钮
	close_button := widget.NewButton("Close Connect", func() {
		if f_tag && sign_in {
			Close_cmd()
			Init_Console(w)
	...
	button1 := container.NewVBox(connect_button, close_button, refresh_button)
	...
	//本地端口输入成功画面
	local_suc_Text := canvas.NewText("Sucess!You will return to Channel Lists in two seconds.", color.Black)
	content_suc_loc := container.New(layout.NewCenterLayout(), local_suc_Text)
	...
	Local_Port_Button := widget.NewButton("OK", func() {
		if len(entry_loc_port.Text) > 0 {
			Change_Text(entry_loc_port.Text)
			w.SetContent(content_suc_loc)
